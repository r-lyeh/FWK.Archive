...
//      1.7.7       Geometry Shaders. External preamble files. //begin//end labels for geom/comp shaders. //< @r-lyeh
//
...
struct cmd_args {
    ...
    const char* preamble; int replace_preamble; //< @r-lyeh
};
    ...
    int profile_version, EShLanguage stage, int debug)
{
    ...
    case EShLangGeometry:
        sx_snprintf(target, sizeof(target), "gs_%d_%d", major_ver, minor_ver);
        break;
    default:
    //< @r-lyeh
    case EShLangVertex:
...
static int compile_files(cmd_args& args, const TBuiltInResource& limits_conf)
{
    ...
    if ((args.vs_filepath || args.fs_filepath) && args.vs_filepath == args.fs_filepath) {
        ...
        while (*text) {
            if (sx_strnequal(text, "//@begin_", 9)) {
                ...
                //< @r-lyeh
                #define PARSE_BLOCK(shaderMode) \
                    text += (text[4]=='\r'&&text[5]=='\n') ? 6 : 5; \
                    const char* end_block = find_end_block(text); \
                    if (!end_block) { \
                        printf("unbalanced //@begin_* //@end_* pair\n"); \
                        return -1; \
                    } \
                    compile_file_desc d = { \
                        shaderMode, \
                ...
                if (sx_strnequal(text, "vert", 4) && (text[4]=='\n' || (text[4]=='\r'&&text[5]=='\n'))) {
                    PARSE_BLOCK(EShLangVertex);
                } else if (sx_strnequal(text, "frag", 4) && (text[4]=='\n' || (text[4]=='\r'&&text[5]=='\n'))) {
                    PARSE_BLOCK(EShLangFragment);
                } else if (sx_strnequal(text, "comp", 4) && (text[4]=='\n' || (text[4]=='\r'&&text[5]=='\n'))) {
                    PARSE_BLOCK(EShLangCompute);
                } else if (sx_strnequal(text, "geom", 4) && (text[4]=='\n' || (text[4]=='\r'&&text[5]=='\n'))) {
                    PARSE_BLOCK(EShLangGeometry);
                } else {
                    printf("invalid @begin tag found\n");
                }
        ...
        //< @r-lyeh
        if (args.gs_filepath) {
            compile_file_desc d = { EShLangGeometry, args.gs_filepath };
            sx_array_push(g_alloc, files, d);
        }
        //< @r-lyeh
    }
    ...
    for (int i = 0; i < sx_array_count(files); i++) {
        ...
        if( args.replace_preamble ) def = args.preamble; //< @r-lyeh
        else if( args.preamble ) def = std::string(args.preamble) + def; //< @r-lyeh
...
static void detect_input_file(cmd_args* args, const char* file)
{
    ...
    if (sx_strequalnocase(ext, ".vert") || sx_strequalnocase(ext, ".vs")) { //< @r-lyeh: add .vs
        args->vs_filepath = file;
    } else if (sx_strequalnocase(ext, ".frag") || sx_strequalnocase(ext, ".fs")) { //< @r-lyeh: add .fs
        args->fs_filepath = file;
    } else if (sx_strequalnocase(ext, ".comp") || sx_strequalnocase(ext, ".cs")) { //< @r-lyeh: add .cs
...
int main(int argc, char* argv[])
{
    ...
    const sx_cmdline_opt opts[] = {
        ...
        { "comp", 'c', SX_CMDLINE_OPTYPE_REQUIRED, 0x0, 'c', "Compute shader source file", "Filepath" }, //< @r-lyeh: compute>comp
        { "geom", 'g', SX_CMDLINE_OPTYPE_REQUIRED, 0x0, 'g', "Geometry shader source file", "Filepath" }, //< @r-lyeh
        ...
        { "debug", 'd', SX_CMDLINE_OPTYPE_FLAG_SET, &args.debug_bin, 1, "Generate debug info for binary compilation, should come with --bin", 0x0 }, //< @r-lyeh 'g'>'d'
        ...
        { "prepend-preamble", 'B', SX_CMDLINE_OPTYPE_REQUIRED, 0x0, 'B', "Prepend shader file to preamble before processing", "Filepath"}, //< @r-lyeh
        { "replace-preamble", 'R', SX_CMDLINE_OPTYPE_REQUIRED, 0x0, 'R', "Replace preamble with shader file before processing", "Filepath"}, //< @r-lyeh
        SX_CMDLINE_OPT_END
    };
    ...
    while ((opt = sx_cmdline_next(cmdline, NULL, &arg)) != -1) {
        ...
        case 'E':
            ...
            break;
        case 'R': case 'B': //< @r-lyeh
            args.replace_preamble = opt == 'R'; //< @r-lyeh
            args.preamble = arg; //< @r-lyeh
            break; //< @r-lyeh
        default:
            break;
        }
    ...
    //< @r-lyeh
    if (args.preamble && !sx_os_path_isfile(args.preamble)) {
        puts("Preamble file is invalid");
        exit(-1);
    }
    if (args.preamble) {
        sx_mem_block* mem = sx_file_load_text(g_alloc, args.preamble); // @leak
        args.preamble = (const char*)mem->data;
    }
    //< @r-lyeh

    if ((args.vs_filepath && !sx_os_path_isfile(args.vs_filepath)) || (args.fs_filepath && !sx_os_path_isfile(args.fs_filepath)) || (args.cs_filepath && !sx_os_path_isfile(args.cs_filepath)) || (args.gs_filepath && !sx_os_path_isfile(args.gs_filepath))) { //< @r-lyeh: add gs
        puts("Input files are invalid");
        exit(-1);
    }

    if (!args.vs_filepath && !args.fs_filepath && !args.cs_filepath && !args.gs_filepath) { //< @r-lyeh: add gs
    ...
    //< @r-lyeh
    if (args.gs_filepath && (args.vs_filepath || args.fs_filepath)) {
        puts("Cannot link geometry-shader with either fragment shader or vertex shader");
        exit(-1);
    }
    //< @r-lyeh
