...
//    printf("searching at %s\n", path);
    ...
    for( int i = 0; list[i]; ++i ) {
        ...
        char *absolute = file_pathabs(va("%s/%s", path, list[i])); // ../dir/./file.ext -> c:/prj/dir/file.ext
//        printf("%s->%s %d\n", list[i], absolute, file_directory(absolute) );

        if( file_directory(absolute) ) {
            const char *dir = absolute[ strlen(absolute) - 1 ] == '/' ? va("%.*s", (int)strlen(absolute) - 1, absolute) : absolute; // remove last '/' if present
...
static int browser_run(struct nk_context *ctx, struct browser *browser, int windowed, struct nk_rect total_space) {
    ...
    if( nk_group_begin(ctx, "Special", NK_WINDOW_NO_SCROLLBAR) ) {
        ...
        if (nk_button_image_label(ctx,media.custom_folders[BROWSER_HOME],"Home",NK_TEXT_CENTERED))
            browser_chdir_and_reload_directory_content(browser, browser->home);

        if (nk_button_image_label(ctx,media.custom_folders[BROWSER_DESKTOP],"Desktop",NK_TEXT_CENTERED))
            browser_chdir_and_reload_directory_content(browser, browser->desktop);

        if (nk_button_image_label(ctx,media.custom_folders[BROWSER_COMPUTER],"Computer",NK_TEXT_CENTERED))
            browser_chdir_and_reload_directory_content(browser, browser->computer);

        if (nk_button_image_label(ctx,media.custom_folders[BROWSER_PROJECT],"Project",NK_TEXT_CENTERED))
            browser_chdir_and_reload_directory_content(browser, browser->project);
...
const int icon_height = 67; // icon height (96) + button padding (??). originally: 135
        ...
        if (index != -1) {
// printf("%s + %s = ", browser->directory, browser->directories[index]);
