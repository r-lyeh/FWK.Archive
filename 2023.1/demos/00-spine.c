...
spine_t spine(const char *file_json, const char *file_atlas, unsigned flags) {
    ...
    data_push(vfs_read(file_json)); // @fixme: data_push_from_file() ?

    array_resize(t.bones, data_count("/bones"));
    array_reserve(t.slots, data_count("/slots"));
    array_resize(t.skins, data_count("/skins"));
    array_resize(t.anims, data_count("/animations"));

    for( int i = 0, end = data_count("/bones"); i < end; ++i ) {
        spine_bone_t v = {0};
        v.name = STRDUP(data_string("/bones[%d]/name", i));
    ...
    for( int i = 0, end = data_count("/slots"); i < end; ++i ) {
        spine_slot_t v = {0};
        v.name = STRDUP(data_string("/slots[%d]/name", i));
        v.bone = STRDUP(data_string("/slots[%d]/bone", i));
        v.attach = STRDUP(data_string("/slots[%d]/attachment", i));
    ...
    for( int i = 0, end = data_count("/skins"); i < end; ++i ) {
        spine_skin_t v = {0};
        v.name = STRDUP(data_string("/skins[%d]/name", i));

        for( int j = 0, jend = data_count("/skins[%d]/attachments",i); j < jend; ++j ) // /skins/default/
        for( int k = 0, kend = data_count("/skins[%d]/attachments[%d]",i,j); k < kend; ++k ) { // /skins/default/left hand item/
            spine_rect_t r = {0};
    ...
    for( int i = 0, end = data_count("/animations"); i < end; ++i ) {
        int id;
        const char *name;

        spine_anim_t v = {0};
        v.name = STRDUP(data_key("/animations[%d]", i));

        // slots / attachments

        for( int j = 0, jend = data_count("/animations[%d]/slots",i); j < jend; ++j )
        for( int k = 0, kend = data_count("/animations[%d]/slots[%d]",i,j); k < kend; ++k ) // ids
        {
            int bone_id = find_bone_id(&t, data_key("/animations[%d]/bones[%d]",i,j));
            if( bone_id < 0 ) continue;

            for( int l = 0, lend = data_count("/animations[%d]/slots[%d][%d]",i,j,k); l < lend; ++l ) { // channels (rot,tra,attach)
                spine_animkey_t key = {0};

                key.name = STRDUP(data_string("/animations[%d]/slots[%d][%d][%d]/name",i,j,k,l));
                key.time = data_float("/animations[%d]/slots[%d][%d][%d]/time",i,j,k,l);
                if( data_count("/animations[%d]/slots[%d][%d][%d]/curve",i,j,k,l) == 4 ) {
                key.curve[0] = data_float("/animations[%d]/slots[%d][%d][%d]/curve[0]",i,j,k,l);
                key.curve[1] = data_float("/animations[%d]/slots[%d][%d][%d]/curve[1]",i,j,k,l);
                key.curve[2] = data_float("/animations[%d]/slots[%d][%d][%d]/curve[2]",i,j,k,l);
                key.curve[3] = data_float("/animations[%d]/slots[%d][%d][%d]/curve[3]",i,j,k,l);
                }
        ...
        for( int j = 0, jend = data_count("/animations[%d]/bones",i); j < jend; ++j ) // slots or bones
        for( int k = 0, kend = data_count("/animations[%d]/bones[%d]",i,j); k < kend; ++k ) { // bone ids
            int bone_id = find_bone_id(&t, data_key("/animations[%d]/bones[%d]",i,j));
            if( bone_id < 0 ) continue;

            // parse bones
            for( int l = 0, lend = data_count("/animations[%d]/bones[%d][%d]",i,j,k); l < lend; ++l ) { // channels (rot,tra,attach)
                const char *channel = data_key("/animations[%d]/bones[%d][%d]",i,j,k);
                int track = !strcmp(channel, "rotate") ? 1 : !strcmp(channel, "translate") ? 2 : 0;
                if( !track ) continue;

                spine_animkey_t key = {0};

                key.time = data_float("/animations[%d]/bones[%d][%d][%d]/time",i,j,k,l);
                if( data_count("/animations[%d]/bones[%d][%d][%d]/curve",i,j,k,l) == 4 ) {
                key.curve[0] = data_float("/animations[%d]/bones[%d][%d][%d]/curve[0]",i,j,k,l);
                key.curve[1] = data_float("/animations[%d]/bones[%d][%d][%d]/curve[1]",i,j,k,l);
                key.curve[2] = data_float("/animations[%d]/bones[%d][%d][%d]/curve[2]",i,j,k,l);
                key.curve[3] = data_float("/animations[%d]/bones[%d][%d][%d]/curve[3]",i,j,k,l);
                }
                ...
                key.y = data_float("/animations[%d]/bones[%d][%d][%d]/y",i,j,k,l),
