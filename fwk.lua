--autogenerated luajit bindings. do not edit. 2021/09/22
...
//lcpp INF [0000] ptr: macro name but used as C declaration in:static inline uint64_t hash_ptr(const void *ptr) {
//lcpp INF [0000] ptr: macro name but used as C declaration in: inline uint64_t hash_ptr(const void *ptr) {
//lcpp INF [0000] ptr: macro name but used as C declaration in:uint64_t key = (uint64_t)(uintptr_t)ptr;
...
WINDOW_LANDSCAPE =128,
 ...
 inline int less_int(int a, int b) {
return a - b;
}
 inline int less_u64(uint64_t a, uint64_t b) {
return a > b ? +1 : -!!(a - b);
}
 inline int less_ptr(void *a, void *b) {
return (uintptr_t)a > (uintptr_t)b ? +1 : -!!((uintptr_t)a - (uintptr_t)b);
}
 inline int less_str(char *a, char *b) {
return strcmp((const char *)a, (const char *)b);
}
 inline uint32_t unhash_32(uint32_t x) {
x = ((x >> 16) ^ x) *295559667;
x = ((x >> 16) ^ x) *295559667;
x = (x >> 16) ^ x;
return x;
...
x = (x ^ (x >> 30)) * UINT64_C(4294967295);
x = (x ^ (x >> 27)) * UINT64_C(4294967295);
x = x ^ (x >> 31);
return x;
}
 inline uint64_t hash_flt(double x) {
union { double d; uint64_t i; } c;
return c.d = x, hash_64(c.i);
}
 inline uint64_t hash_str(const char* str) {
uint64_t hash = 14695981039346656037;
while( *str ) hash = ( (unsigned char)*str++ ^ hash ) *4294967295;
return hash;
}
 inline uint64_t hash_int(int key) {
return hash_32((uint32_t)key);
}
 inline uint64_t hash_ptr(const void *ptr) {
uint64_t key = (uint64_t)(uintptr_t)ptr;
return hash_64(key >> 3);
}
 inline void* vrealloc( void* p, size_t sz ) {
if( !sz ) {
if( p ) {
size_t *ret = (size_t*)p - 2;
ret[0] = 0;
ret[1] = 0;
(len1_ = (0), (len1_ ? (xrealloc((ret),len1_)) : xrealloc((ret),0)));
}
return 0;
} else {
size_t *ret = (size_t*)p - 2;
if( !p ) {
ret = (size_t*)(len1_ = (sizeof(size_t)), (len1_ ? (xrealloc((0),len1_)) : xrealloc((0),0)));
ret[0] = sz;
ret[1] = 0;
} else {
size_t osz = ret[0];
...
ret = (size_t*)(len1_ = (sizeof(size_t)), (len1_ ? (xrealloc((ret),len1_)) : xrealloc((ret),0)));
ret[0] = sz;
