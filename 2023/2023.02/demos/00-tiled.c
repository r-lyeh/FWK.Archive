...
void tiled_ui(tiled_t *p) {
    ...
    for( int i = 0; i < array_count(t.layers); ++i ) {
        if( ui_label2_toolbar(va("- %s (%dx%d)", t.names[i], t.layers[i].cols, t.layers[i].rows ), t.visible[i] ? "\xee\xa3\xb4" : "\xee\xa3\xb5", 24.f) > 0 ) { // ICON_MD_VISIBILITY / ICON_MD_VISIBILITY_OFF
            p->visible[i] ^= true;
        }
    }
    ui_separator();
