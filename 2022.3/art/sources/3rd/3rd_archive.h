...
zip* zip_open(const char *file, const char *mode /*r,w,a*/);
    ...
    bool zip_append_file(zip*, const char *entryname, const char *comment, FILE *in, unsigned compr_level);
    bool zip_append_file_timeinfo(zip*, const char *entryname, const char *comment, FILE *in, unsigned compr_level, struct tm *);
...
void *zip_extract(zip *z, unsigned index) { // must free()
    if( z->in && index < z->count ) {
        ...
        char *out = (char*)REALLOC(0, outlen + 1 + EXCESS(flags));
        unsigned ret = zip_extract_data(z, index, out, outlen); if(ret) out[outlen] = '\0';
