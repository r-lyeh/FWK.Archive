...
int SKY_DIRe = 0;
const char *SKY_DIRSe[] = {
    "hdr/Tokyo_BigSight_1k.hdr",
};

int OBJ_MDLe = 0;
const char *OBJ_MDLSe[] = {
...
int main(int argc, char** argv) {
    ...
    while( window_swap()) {
        ...
        if( !initialized ) {
            initialized = 1;
            sky = skybox(flag("--mie") ? 0 : SKY_DIRSe[SKY_DIRe], 0);
            env_probe = skybox(0, 0);
            mdl = model(OBJ_MDLSe[OBJ_MDLe], 0);
        }
        ...
        if( ui_panel("Scene", 0)) {
            if( ui_list("Skybox", SKY_DIRSe, countof(SKY_DIRSe), &SKY_DIRe) ) {
                must_reload = 1;
            }
            if( ui_list("Model", OBJ_MDLSe, countof(OBJ_MDLSe), &OBJ_MDLe) ) {
                must_reload = 1;
            }
            ui_separator();
            for (int i = 0; i < 9; i++) {
