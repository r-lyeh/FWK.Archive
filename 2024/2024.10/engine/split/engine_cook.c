...
cook_script_t cook_script(const char *rules, const char *infile, const char *outfile) {
    ...
    for( int pass = 0; pass < countof(mcs.cs); ++pass ) {
        ...
        cs.gen_ns -= time_ns();
            ...
            map_find_or_add(symbols, "PRETTY", STRDUP(infile + ART_SKIP_ROOT)); // pretty (truncated) input (C:/project/art/file.wav -> file.wav)
...
bool cook_start( const char *cook_ini, const char *masks, int flags ) {
    ...
    do_once {
    #if 0
        const char *HOME = file_pathabs(cook_ini); // ../tools/cook.ini -> c:/project/tools/cook.ini
        if( strbeg(HOME, app_path() ) ) HOME = STRDUP( file_path( HOME += strlen(app_path()) ) ); // -> tools/ @leak
    #else
        char *HOME = STRDUP(file_pathabs(cook_ini)); // ../tools/cook.ini -> c:/project/tools/cook.ini
    ...
    // estimate ART_SKIP_ROOT (C:/project/demos/assets/file.png -> strlen(C:/project/) -> 11)
    {
        array(char*) dirs = 0;
        for each_substring(ART, ",", art_folder) {
