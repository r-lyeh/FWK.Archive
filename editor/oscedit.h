    ...
    if( server_fd >= 0 ) {
        ...
        if( ui_window("editor", &on) ) {
            ...
            if( !map_count(server_vars) ) {
                ...
                int mouse_click = ui_label2("New", va(">" ICON_MD_LOOP));
            ...
            for each_map_ptr_sorted(server_vars, char *, title, osc_variant, msg) {
                ...
                ui_label_icon_enabled = !!msg->edited;
//                vec2 ui_label_icon_clicked_L; // left
//                vec2 ui_label_icon_clicked_R; // right

                char *title_copy = va(ICON_MD_UNDO " " "%s", *title + 1 + title_len + 1); // /player/title -> UNDO_ICON title
                ...
                else if( msg->type == 's' ) { char s[128] = {0}; strncpy(s, msg->live[0].s, sizeof(s)); uint64_t old = hash_str(s); ui_string(title_copy, s, sizeof(s)); uint64_t mod = hash_str(s); if(mod != old) msg->edited |= 1, FREE(msg->live[0].s), msg->live[0].s = stringf("%s",s); }
